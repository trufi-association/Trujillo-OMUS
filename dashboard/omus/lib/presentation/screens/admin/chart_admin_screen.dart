...
/// Screen for loading chart configuration data.
class ChartAdminScreen extends StatefulWidget {
  const ChartAdminScreen({super.key});

  @override
  State<ChartAdminScreen> createState() => _ChartAdminScreenState();
}
...
class _ChartAdminScreenState extends State<ChartAdminScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return _ChartAdminEditor(data: _data!);
  }
}

/// Editor widget for chart configuration.
class _ChartAdminEditor extends StatefulWidget {
  final Map<String, dynamic> data;

  const _ChartAdminEditor({required this.data});

  @override
  State<_ChartAdminEditor> createState() => _ChartAdminEditorState();
}

class _ChartAdminEditorState extends State<_ChartAdminEditor> {
  ...
  void _initializeControllers() {
    for (final sectionKey in _editableData.keys) {
      final items =
          _editableData[sectionKey]['items'] as List<dynamic>? ?? [];
      for (int i = 0; i < items.length; i++) {
        final key = '$sectionKey-$i';
        _titleControllers[key] =
            TextEditingController(text: items[i]['title']);
        _descriptionControllers[key] =
            TextEditingController(text: items[i]['description']);
        _urlControllers[key] =
            TextEditingController(text: items[i]['url']);
      }
    }
  }
  ...
  Future<void> _saveChanges() async {
    try {
      final response = await ApiHelper.post(
        path: '/ChartConfig/update',
        body: jsonEncode(_editableData),
        useToken: true,
      );
      if (response.statusCode == 200) {
        ...
        child: Column(
          ...
          children: [
            ...
            TextField(
              ...
              onChanged: (value) =>
                  _updateItem(sectionKey, index, 'title', value),
            ),
            const Text(
              'Description:',
              style: TextStyle(fontWeight: FontWeight.bold),
            ...
            TextField(
              ...
              onChanged: (value) =>
                  _updateItem(sectionKey, index, 'description', value),
            ),
              ...
              children: [
                Expanded(
                  child: TextField(
                    ...
                    onChanged: (value) =>
                        _updateItem(sectionKey, index, 'url', value),
                ...
                DropdownButton<String>(
                  value: item['type'],
                  onChanged: (newValue) =>
                      _updateItem(sectionKey, index, 'type', newValue!),
